...
local function createAdvancedStaffWindow()
    ...
    local function fetchAndShow()
        ...
        task.spawn(function()
            ...
            -- Method A: batch presence POST
            local ids = {}
            for _, s in ipairs(staffList) do
                table.insert(ids, s.userId)
            end

            local presenceBody = HttpService:JSONEncode({ userIds = ids })
            local presRes = httpRequest({
            ...
            if presRes.Success then
                local pOk, pData = pcall(function()
                    return HttpService:JSONDecode(presRes.Body)
                end)
                if pOk and pData and pData.userPresences then
            ...
            -- Method B: if batch failed / incomplete, try per-user (list is tiny)
            local missing = {}
            for _, s in ipairs(staffList) do
                if not presenceMap[s.userId] then
                    table.insert(missing, s.userId)
                end
            end

            if #missing > 0 then
                statusLabel.Text = "Presence fallback (" .. #missing .. " left)..."
                for _, uid in ipairs(missing) do
                    -- B1: single-user presence POST
                    ...
                    -- B2: old online-status endpoint as last resort
                    if not presenceMap[uid] then
                        ...
                        if oldRes.Success then
                            ...
                            if ok and type(data) == "table" then
                                -- map old format into something usable
                    ...
                    task.wait(0.15) -- be gentle on rate limits
                end
            end
            ...
            if next(presenceMap) == nil then
                statusLabel.Text = "Presence unavailable on this executor – showing staff only"
                warn("[Staff] All presence methods failed. Staff list still shown without online status.")
            end
