...
-- Important staff roles (by name)
local STAFF_ROLES = {
    ["Owner"] = { priority = 100, color = Color3.fromRGB(255, 200, 80) },
    ["Co Owner"] = { priority = 90, color = Color3.fromRGB(255, 140, 60) },
    ["Developer"] = { priority = 85, color = Color3.fromRGB(255, 120, 80) },
    ["Head Moderator"] = { priority = 70, color = Color3.fromRGB(230, 80, 80) },
    ["Supervisor"] = { priority = 60, color = Color3.fromRGB(230, 150, 60) },
    ["Moderator"] = { priority = 50, color = Color3.fromRGB(80, 160, 255) },
    ["Game Tester"] = { priority = 40, color = Color3.fromRGB(80, 210, 200) },
    ["Contributor"] = { priority = 30, color = Color3.fromRGB(100, 210, 120) },
    ["Notable Builder"] = { priority = 25, color = Color3.fromRGB(160, 120, 255) },
    ["Trusted User"] = { priority = 10, color = Color3.fromRGB(140, 140, 150) }, -- troll bait
}
-- Roles we never treat as staff
local IGNORED_ROLES = {
    ["Guest"] = true,
}
...
local function isStaffRole(roleName)
    ...
    if IGNORED_ROLES[roleName] then return false end
    return STAFF_ROLES[roleName] ~= nil
end

local function getRoleStyle(roleName)
    return STAFF_ROLES[roleName] or { priority = 0, color = Color3.fromRGB(140, 140, 140) }
end
...
local function updatePlayerList()
    ...
    task.spawn(function()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if isStaffRole(roleName) then
                local style = getRoleStyle(roleName)
        ...
        if count == 0 then
            ...
            for i = 1, math.min(count, maxShow) do
                ...
                local textColor = (data.Player == LocalPlayer) and PRIMARY_TEXT or (data.Color or STAFF_TEXT)
...
local function createAdvancedStaffWindow()
    ...
    local currentSortMode = "status" -- status | offline | name | role | rank
    local applySortAndPopulate -- forward declare for dropdown
    ...
    local sortOptions = {
        ...
        { id = "role", label = "Role" },
        { id = "rank", label = "Rank (Highest first)" },
    }
    local sortLabels = {}
    for _, o in ipairs(sortOptions) do sortLabels[o.id] = o.label end

    local sortDropBtn = Instance.new("TextButton")
    sortDropBtn.Size = UDim2.new(0, 140, 0, 18)
    sortDropBtn.Position = UDim2.new(0, 8, 0, 27)
    sortDropBtn.BackgroundColor3 = Color3.fromRGB(32, 32, 38)
    sortDropBtn.BorderSizePixel = 0
    sortDropBtn.Text = "Sort: " .. (sortLabels[currentSortMode] or "Status")
    sortDropBtn.Font = Enum.Font.Code
    sortDropBtn.TextSize = 10
    sortDropBtn.TextColor3 = Color3.fromRGB(180, 200, 160)
    sortDropBtn.TextXAlignment = Enum.TextXAlignment.Left
    sortDropBtn.Parent = winMain

    local sortDropStroke = Instance.new("UIStroke")
    sortDropStroke.Thickness = 1
    sortDropStroke.Color = Color3.fromRGB(55, 55, 60)
    sortDropStroke.Parent = sortDropBtn

    local sortDropPad = Instance.new("UIPadding")
    sortDropPad.PaddingLeft = UDim.new(0, 6)
    sortDropPad.Parent = sortDropBtn

    local sortMenu = Instance.new("Frame")
    sortMenu.Size = UDim2.new(0, 140, 0, #sortOptions * 18)
    sortMenu.Position = UDim2.new(0, 8, 0, 46)
    sortMenu.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    sortMenu.BorderSizePixel = 0
    sortMenu.Visible = false
    sortMenu.ZIndex = 10
    sortMenu.Parent = winMain

    local sortMenuStroke = Instance.new("UIStroke")
    sortMenuStroke.Thickness = 1
    sortMenuStroke.Color = Color3.fromRGB(55, 55, 60)
    sortMenuStroke.Parent = sortMenu
    ...
    for i, opt in ipairs(sortOptions) do
        ...
        optBtn.MouseButton1Click:Connect(function()
            currentSortMode = opt.id
            sortDropBtn.Text = "Sort: " .. opt.label
            sortMenu.Visible = false
            if applySortAndPopulate then
                applySortAndPopulate()
            end
        end)
        optBtn.MouseEnter:Connect(function()
            optBtn.BackgroundColor3 = Color3.fromRGB(40, 45, 40)
            optBtn.TextColor3 = Color3.fromRGB(180, 230, 160)
        end)
    ...
    sortDropBtn.MouseButton1Click:Connect(function()
        sortMenu.Visible = not sortMenu.Visible
    end)

    local statusLabel = Instance.new("TextLabel")
    statusLabel.Size = UDim2.new(1, -160, 0, 16)
    statusLabel.Position = UDim2.new(0, 155, 0, 28)
    ...
    local function sortStaffList(list)
        local statusOrderOnline = {
            InGame = 1, InStudio = 2, Online = 3, Invisible = 4, Offline = 5, Unknown = 6
        }
        local statusOrderOffline = {
            Offline = 1, Unknown = 2, Invisible = 3, Online = 4, InStudio = 5, InGame = 6
        }
        ...
        table.sort(list, function(a, b)
            ...
            elseif currentSortMode == "role" then
                ...
                return a.username:lower() < b.username:lower()
            elseif currentSortMode == "rank" then
                local ra, rb = a.priority or a.rank or 0, b.priority or b.rank or 0
                if ra ~= rb then return ra > rb end
                return a.username:lower() < b.username:lower()
            elseif currentSortMode == "offline" then
                local oa = statusOrderOffline[a.status] or 9
                local ob = statusOrderOffline[b.status] or 9
                if oa ~= ob then return oa < ob end
                return a.username:lower() < b.username:lower()
            else -- status (online first)
                local oa = statusOrderOnline[a.status] or 9
                local ob = statusOrderOnline[b.status] or 9
                if oa ~= ob then return oa < ob end
                return a.username:lower() < b.username:lower()
            end
        end)
    ...
    local function populate(staffData)
        ...
        for _, entry in ipairs(staffData) do
            local style = getRoleStyle(entry.role)
            local accent = style.color
            -- Tint card background toward role color
            local cardBg = Color3.fromRGB(
                math.floor(18 + accent.R * 40),
                math.floor(18 + accent.G * 40),
                math.floor(18 + accent.B * 40)
            )
            local strokeCol = Color3.fromRGB(
                math.floor(accent.R * 180),
                math.floor(accent.G * 180),
                math.floor(accent.B * 180)
            )
            ...
            nameLbl.TextColor3 = getRoleStyle(entry.role).color
            nameLbl.TextSize = 12
                ...
                repeat
                    ...
                    for _, user in ipairs(memData.data) do
                        if user.userId and not userIdSet[user.userId] then
                            ...
                            local style = getRoleStyle(role.name)
